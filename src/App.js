...
function App(){
    const [itemArray , updateItemArray] = useState(["empty","empty","empty","empty","empty","empty","empty","empty","empty"])
    ...
    function checkIsWinner(){
        if(itemArray[0] === itemArray[1] && itemArray[0] === itemArray[2] && itemArray[0] !== "empty"){
            ...
            setWinMessage(`${itemArray[0]} Wins`);
            toast(`${itemArray[0]} Wins`, {type : "success"})
        }else if(itemArray[2] === itemArray[4] && itemArray[2] === itemArray[6] && itemArray[0] !== "empty"){
            setWinMessage(`${itemArray[0]} Wins`);
            toast(`${itemArray[0]} Wins` , {type : "success"})
        }
    }
    ...
    return(
        ...
        <div className="gridContainer">
            {itemArray.map((value , index)=>(
                <div className={itemArray[index] === "empty" ? "item_available" : "item_unavailable"} onClick={(curBtn)=>changeItem(index , curBtn)}>
                    <Icon name={itemArray[index]}  />
                </div>
            ))}
        </div>
        <h1 className="text-center">{isCross ? "Cross Turns " : "Circle Turns"}</h1>
